...
def register(key, proxy):
    ...
    try:
        ...
        try:
            ...
            utlities.s_print(f"Captcha key is invalid or invalid response ...")
            exit(1)
        ...
        if response.status_code == 403:
            utlities.s_print(f"New locked token genrated {token}")
            tokenCreatedLocked+=1
            os.system(
                "TITLE Tokens created : "
                + str(tokensCreated)
                +", Token locked : "+ str(tokenCreatedLocked)
            ...
            exit(1)
        utlities.s_print(f"New unlocked token genrated {token}")
...
def start():
    while True:
        try:
            ...
            if config["useAntiCaptcha"]:
                #   Getr captcha key from anti captcha
                solver = hCaptchaProxyon()
                solver.set_verbose(0)
                solver.set_key(config["antiCaptchaApiKey"])
                ...
                ip = socket.gethostbyname(proxyFormated.split("@")[1].split(":")[0] )
                proxyFormated = proxyFormated.split("@")
                proxyFormated[1] =ip+ ":"+proxyFormated[1].split(":")[1]
                proxyFormated =   "@".join(  proxyFormated)
                if "@" in proxyFormated:
                    solver.set_proxy_address(ip  )
                    solver.set_proxy_port(int(proxyFormated.split("@")[1].split(":")[1] ))
                    solver.set_proxy_login( proxyFormated.split("@")[0].split(":")[0]         )
                    solver.set_proxy_password(proxyFormated.split("@")[0].split(":")[1] )
                else:
                    solver.set_proxy_address(ip)
                    solver.set_proxy_port(int(proxyFormated.split(":")[1]))
                key = solver.solve_and_return_solution()
            else:
...
def mainhandler():
    try:
        threads = int(input("Put a thread number : "))
    except:
